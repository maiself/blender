...
bool MeshManager::displace_subpatch(Device *device, DeviceScene *dscene, Scene *scene, Mesh *mesh, Progress& progress,
		TessellatedSubPatch* subpatch, int subpatch_id)
{
	...
	/* setup input for device task */
	const size_t num_verts = subpatch->num_verts;
	device_vector<uint4> d_input;
	uint4 *d_input_data = d_input.resize(num_verts);
	size_t d_input_size = 0;
	...
	for(size_t i = 0; i < num_verts; i++) {
		...
		/* when used, non-instanced convention: object = ~object */
		int object = object_index;
		int prim = ~subpatch_id;

		/* back */
		uint4 in = make_uint4(object, prim, i, 0);
		d_input_data[d_input_size++] = in;
	}
	...
	for(size_t i = 0; i < num_verts*2; i += 2) {
		float3 off = float4_to_float3(offset[k++]);
